...
# Created: 05/17/2024
# CRUD using clever-cloud - Final Examination v1.4
...
mydb = mysql.connector.connect(
    host = "bmgp4xvtce7tybmdhn3d-mysql.services.clever-cloud.com",
    user = "um6byu2maf8889ny",
    password = "GlC6uywvNVDGCJf2q5rZ",
    database = "bmgp4xvtce7tybmdhn3d"
)
...
def encrypt_message(message, key):
    ...
    encrypted_message = ciphertext.encrypt(message.encode())
    return encrypted_message

def decrypt_message(encrypted_message, key):
    ciphertext = Fernet(key)
    decrypted_message = ciphertext.decrypt(encrypted_message)
...
def Save():
    ...
    if entry_Sex.get() not in {"Male", "Female"}:
        ...
        # Data Encryption
    encrypted_FirstName = encrypt_message(entry_FirstName.get(), encryption_key)
    encrypted_LastName = encrypt_message(entry_LastName.get(), encryption_key)
    encrypted_Age = encrypt_message(entry_Age.get(), encryption_key)
    encrypted_Sex = encrypt_message(entry_Sex.get(), encryption_key)
    ...
    if validate_input(entry_Search, "Student ID", is_integer=True, integer_range=(1, 9999999)):
        try:
            ...
            if results is not None:
                ...
                ClearEntries()
                    # Data Decryption
                entry_FirstName.insert(0, decrypt_message(results[1], encryption_key))
                entry_LastName.insert(0, decrypt_message(results[2], encryption_key))
                entry_Age.insert(0, decrypt_message(results[3], encryption_key))
                entry_Sex.insert(0, decrypt_message(results[4], encryption_key))
                DisableEntries()
...
root = Tk()
root.title("Student Database v1.4")
